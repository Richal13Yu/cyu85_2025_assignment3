var createScene = function () {
    ...
    */
    ...
    {
        // Direction to light and distance
        vec3  L    = normalize(lPos - vPosW);
        float dist = length(lPos - vPosW);

        // Smooth attenuation (tweak factors as you like)
        ...
        // Specular term (Phong: reflect about the normal)
        vec3  R = reflect(-L, N);
        float rdotv = max(dot(R, V), 0.0);
        vec3  specular = ks * lColor * pow(rdotv, shin);

        // No specular when light is behind the surface
    ...
    // Material
    //1)
    shaderMat.setFloat("uMatAmbient", 0.10);
    shaderMat.setColor3("uMatDiffuse", new BABYLON.Color3(0.20, 0.60, 0.20));    shaderMat.setColor3("uMatSpecular", new BABYLON.Color3(1, 1, 1));
    //2)
    shaderMat.setColor3("uMatSpecular", new BABYLON.Color3(0.9, 0.9, 0.9));
    shaderMat.setFloat("uMatShininess", 128.0);
    //3)
    shaderMat.setColor3("uPointLightColor", new BABYLON.Color3(1.0, 0.55, 0.35));
    shaderMat.setVector3("uPointLightPos", new BABYLON.Vector3(-2.5, 3.5, -1.0));
